(*Written by: Charles Booth and Justin Duran*)
...
and
  ...
  | Proj(e,id) ->
    eval_expr e >>=
    fields_of_recordVal >>= fun v ->
    let rec proj_field: (string * (bool * exp_val)) list -> exp_val ea_result = 
    fun n ->
    match n with 
    | [] -> error "Proj Error: Field does not exist"
    | h::t ->
    if (fst h) = id
    then
    (if fst (snd h)
    then int_of_refVal (snd(snd h)) >>= Store.deref g_store
    else return @@ snd (snd h))
    else proj_field t
    in proj_field v
  ...
  | SetField(e1,id,e2) -> 
    eval_expr e1 >>=
    fields_of_recordVal >>= fun v ->
    let rec s_field: (string *(bool * exp_val)) list -> exp_val ea_result =
    fun n ->
    match n with 
    | [] -> error "SetField Error: Field does not exist"
    | h::t ->
    if (fst h) = id
    then
    (if fst (snd h)
    then
    int_of_refVal (snd(snd h)) >>= fun x ->
    eval_expr e2 >>= Store.set_ref g_store x >>= fun _ ->
    return UnitVal
  ...
  | IsNumber(e) ->
    eval_expr e >>= fun n ->
    is_numVal n >>= fun x -> return (BoolVal x)
